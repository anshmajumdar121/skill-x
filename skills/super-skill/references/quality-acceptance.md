...
```text
Given a tool-dependent action (including a delegated subagent's
work),
when completion is claimed,
then a successful tool result or orchestrator-level state
...
- **(SuperSkill) It claims a subagent's report was correct without orchestrator vetting**
...
**For delegation mode, add layer 0**: **Orchestrator Vet** —
reopen ≥ 1 cited file per subagent, confirm line refs, review
final diff. Without this, layers 1-8 may all be false.
...
| **T-09 (delegation)** | **Subagent report vetting** | Reopen cited files, confirm line refs, review diff | Subagent claims match reality | Every cited file matches; every line ref valid | **Orchestrator** |
...
19. **(SuperSkill)** Which subagent report was accepted without being vetted?
20. **(SuperSkill)** Which subagent's silent assumption leaked into the final output?
...
8. **(SuperSkill)** Re-vet any subagent report that didn't survive the review
...
- [ ] **(SuperSkill)** Mode (rigor or delegation) was the right choice
...
- [ ] **(SuperSkill)** Every cited file was reopened and confirmed
- [ ] **(SuperSkill)** Every line ref was validated
...
- [ ] **(SuperSkill)** Subagent reports were vetted (T-09 passed)
