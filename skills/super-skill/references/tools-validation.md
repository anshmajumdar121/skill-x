...
**For delegation mode, add:**
- Is this work parallelizable?
- Can a cheap subagent do this with the same correctness?
- What's the cost vs time savings vs the orchestration overhead?
...
| **(SuperSkill) Subagent** | Parallelizable, token-heavy, codebase-heavy work | **Orchestrator Vet** (T-09): reopen cited files, confirm line refs, review diff | Subagent lies, scope drift, silent defaults, conflicting reports |
...
**For delegation:** don't dispatch a subagent for work that a single
orchestrator call could do in less time than the handoff packet takes
to write.
...
**For subagents:** dispatch with a handoff packet → collect report →
**vet (reopen files, confirm line refs, review diff)** before accepting
→ only then incorporate.

## 5. Parallelism

Parallelize only independent actions. Do not run actions in parallel
when one depends on the result, identifier, decision, or side effect
of another.

**For subagents:** parallelize independent slices. If subagent 2 needs
subagent 1's output, sequence them explicitly or combine.
...
| **(SuperSkill) Subagent report wrong** | Reject, redispatch with corrected handoff packet, or fall back to orchestrator doing the work |
...
- **(SuperSkill) Vet record** — which file was reopened, which line ref was confirmed, which diff was reviewed
...
4. **(SuperSkill) Vet any subagent whose report was incorporated into the corrected artifact**
